...
export function WeekPickerModal({ picker, setPicker, date, currentMeal, query, setQuery, simpleFood, setSimpleFood, netOpen, toggleNet, importStatus, link, setLink, recipes, photoStyle, recipeMeta, close, addSimpleFood, chooseNoStarter, searchWeb, importCopied, importLink, chooseRecipe }: {
  ...
  return <div className="modal-backdrop" onClick={close}><section className="scan-modal week-picker" onClick={(event) => event.stopPropagation()}>
    ...
    {netOpen && <div className="week-net-panel"><p><b>Quelle recette veux-tu chercher ?</b><span>Écris son nom précis, par exemple « gratin dauphinois » ou « côtes de porc au caramel ».</span></p><label className="week-picker-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") searchWeb(); }} placeholder="Nom de la recette" /></label><button className="outline" onClick={searchWeb} disabled={!query.trim()}><Search size={16} /> Chercher « {query.trim() || "ma recette"} » sur Internet</button><small>Sur la recette choisie, touche <b>Partager</b> puis <b>Copier le lien</b>. Reviens ensuite ici.</small><button className="primary" onClick={importCopied}><Plus size={17} /> Importer la recette copiée</button>{importStatus && <small>{importStatus}</small>}<details><summary>Le bouton ne fonctionne pas ?</summary><input value={link.name} onChange={(event) => setLink((current) => ({ ...current, name: event.target.value }))} placeholder="Nom facultatif" /><input type="url" value={link.url} onChange={(event) => setLink((current) => ({ ...current, url: event.target.value }))} placeholder="Coller le lien https://…" /><button className="outline" onClick={importLink} disabled={!link.url.trim()}>Importer ce lien</button></details></div>}
